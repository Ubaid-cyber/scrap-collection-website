...
/**
 * GARHWAL TRADERS - PLATFORM
**/

const translations = {
  en: {
    nav: { home: "Home", pricing: "Pricing", faq: "FAQ", contact: "Contact", login: "Login" },
    hero: {
  ...
  hi: {
    nav: { home: "होम", pricing: "मूल्य निर्धारण", faq: "सामान्य प्रश्न", contact: "संपर्क", login: "लॉगिन" },
    hero: {
...
const BrandCollabs = ({ t }) => {
  ...
  // We double the list to create the "looping" illusion
      ...
      <div className="flex overflow-hidden group">
        <div className="flex animate-marquee whitespace-nowrap">
          {displayBrands.map((i, idx) => (
            <div 
              key={idx} 
              // Added flex-shrink-0 so items don't squish, breaking the animation math
    ...
    <section className="relative h-screen w-full flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center px-6 max-w-4xl">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <Link to="/Pricing.jsx" className="px-8 py-4 bg-green-500 text-white rounded-full font-bold hover:bg-green-400 transition-all shadow-xl flex items-center justify-center gap-2 group">
    ...
    <section className="py-24 bg-white dark:bg-slate-900 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center gap-16">
        <div className="relative w-full md:w-5/12 max-w-sm">
          <div className="absolute inset-0 border-2 border-green-600 rounded-3xl translate-x-4 translate-y-4"></div>
          <div className="relative z-10 aspect-[4/5] overflow-hidden rounded-3xl shadow-2xl bg-slate-100 dark:bg-slate-800">
            <img src="./assets/director.jpeg" alt="Director" className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700" onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1560250097-0b93528c311a?auto=format&fit=crop&w=800&q=80' }} />
          </div>
        </div>
    ...
    <Router>
      <div className={darkMode ? 'dark' : ''}>
        <div className="min-h-screen font-sans text-slate-900 bg-white dark:bg-slate-950 transition-colors duration-500 flex flex-col">
          ...
          {/* Navaligned with directory file names */}
          <nav className={`fixed top-0 w-full z-50 transition-all duration-300 border-b ${scrolled ? 'bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-slate-200 dark:border-slate-800 py-3 shadow-sm' : 'bg-transparent border-transparent py-5'}`}>
            <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
              ...
              <div className="hidden md:flex items-center gap-8 font-bold">
                ...
                <Link to="/Pricing.jsx" className={`hover:text-green-600 transition-colors ${!scrolled ? 'text-white' : 'text-slate-700 dark:text-slate-300'}`}>{t.nav.pricing}</Link>
                <Link to="/FAQ.jsx" className={`hover:text-green-600 transition-colors ${!scrolled ? 'text-white' : 'text-slate-700 dark:text-slate-300'}`}>{t.nav.faq}</Link>
                <Link to="/Contact.jsx" className={`hover:text-green-600 transition-colors ${!scrolled ? 'text-white' : 'text-slate-700 dark:text-slate-300'}`}>{t.nav.contact}</Link>
            ...
            <div className={`md:hidden absolute top-full left-0 w-full bg-white dark:bg-slate-900 border-b transition-all duration-300 ${isMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
              <div className="p-6 flex flex-col gap-4 font-bold">
                ...
                <Link to="/Pricing.jsx" onClick={() => setIsMenuOpen(false)}>{t.nav.pricing}</Link>
                <Link to="/FAQ.jsx" onClick={() => setIsMenuOpen(false)}>{t.nav.faq}</Link>
                <Link to="/Contact.jsx" onClick={() => setIsMenuOpen(false)}>{t.nav.contact}</Link>
              </div>
            </div>
          </nav>
          ...
          <main className="flex-grow">
            <Routes>
              ...
              <Route path="/Pricing.jsx" element={<Pricing t={t} />} />
              ...
              <Route path="/Login" element={
                <div className="pt-40 min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 px-6">
                  <div className="bg-white dark:bg-slate-800 p-12 rounded-3xl shadow-2xl w-full max-w-md border text-center">
                    ...
                    <div className="space-y-4">
                      <input className="w-full p-4 bg-slate-50 dark:bg-slate-700 rounded-xl" placeholder="Username" />
                      <input className="w-full p-4 bg-slate-50 dark:bg-slate-700 rounded-xl" type="password" placeholder="Password" />
          ...
          <footer className="bg-slate-900 text-white py-16">
            <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12 text-center md:text-left">
              <div>
                ...
                <p className="text-slate-400">Making recycling easy across Uttarakhand.</p>
              </div>
              <div className="flex flex-col gap-4">
                <h4 className="font-bold text-green-500 uppercase tracking-widest">Pages</h4>
                <Link to="/Pricing.jsx" className="text-slate-400 hover:text-white">Rates</Link>
                <Link to="/FAQ.jsx" className="text-slate-400 hover:text-white">Help</Link>
                <Link to="/Contact.jsx" className="text-slate-400 hover:text-white">Location</Link>
              </div>
              <div>
                <h4 className="font-bold text-green-500 uppercase tracking-widest mb-6">Contact</h4>
                <p className="text-slate-400">Pauri Market, Uttarakhand<br/>+91 98765 43210</p>
              </div>
            </div>
          </footer>

          <button onClick={toggleDarkMode} className="fixed bottom-6 left-6 z-[60] w-14 h-14 rounded-full bg-green-600 dark:bg-yellow-500 text-white dark:text-slate-900 shadow-2xl flex items-center justify-center transition-all">
